module wot_id::trust {
    ...
    // Error codes
    const E_INVALID_TRUST_LEVEL: u64 = 2;
    const E_NOT_AUTHORIZED: u64 = 1;

    const E_INVALID_EVIDENCE: u64 = 5;
    ...
    public entry fun establish_claim_trust(
        ...
        // Create initial attestation from the verifier
        let initial_attestation = Attestation {
            kind: string::utf8(b"self-assessed"),
            attestor: verifier_did,
            method: string::utf8(b"direct-creation"),
            timestamp: now,
            strength: 100, // Self-attestation is max strength
        };
        
        let attestations = vector::singleton(initial_attestation);
